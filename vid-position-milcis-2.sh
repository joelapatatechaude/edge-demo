...
Y_SIZE_BROWSER=$((1200 - $TITLE_PIXEL_SIZE  - $MENU_PIXEL_SIZE))
...
PI_BEFORE_NAME=pi-NONONO
PI_AFTER_NAME=pi-yolo-NONONO
X_SIZE_PI=$(( 960 - 8 ))
Y_SIZE_PI=$(( 720 - 6 ))
X_TOP_PI_BEFORE=0
...
function create_browser {
    ...
    google-chrome --new-window "$CHROME_URL" --user-data-dir=/tmp/chrome/  >& /dev/null &
    #sleep 0.5
...
function main_ffplay {
    ...
    echo "unsorted list"
    echo "$NAME_LIST"
    SORTE_LIST=$(echo "$NAME_LIST" | sort)
    echo "THE LIST SORTED:"
    echo "$SORTE_LIST"
    for i in $SORTE_LIST; do
